...
import { mutall_error, mymap, } from "../../../schema/v/code/schema.js";
import { myalert } from "../../../schema/v/code/mutall.js";
import { user } from "../../../outlook/v/code/app.js";
import { io_parent } from "./io.js";
import { popup } from "../../../popup/v/code/popup.js";
...
export class authoriser extends popup {
    //
    ...
    sections;
    //
    //The html filename of the authoriser
    static html_file = "authoriser.html";
    //
    // The url of the authoriser page
    static get url() {
        //
        // Pass import.meta.url from *this* module (authoriser.ts)
        return this.get_html_url(this.html_file, import.meta.url);
    }
    ...
    set_sections() {
        ...
        const radios = this.document.querySelectorAll('input[type="radio"]');
        //
        //Add the event listener for the radio buttons
        // add both the click and fluctuate events to the radio buttons
        radios.forEach((radio) => {
            radio.onclick = (evt) => {
                this.clear_errors();
                fluctuate.onclick(radio);
            };
        });
        //
        // Add the fluctuate functionality to the auth form
    ...
    async authorise(event) {
        ...
        if (!user)
            return;
        //
        // Ensure the user object structure matches what's expected by post_data_to_opener
        // and the 'administer' validation.
        const user_data = {
            name: user.name,
            pk: user.pk,
        };
        ...
        //
        // Use the popup method to send data to the parent
        this.send_data_to_parent(user_data);
        //
        // The popup might be closed automatically by post_data_to_opener
        // or you can explicitly close it here if needed.
        window.close();
    }
    //
    // Check if the data returned from the popup is a user or undefined
    static validate_auth_message(data) {
        //
        // Check if the data is a user
        const is_user = typeof data === "object" &&
            data !== null &&
            "name" in data &&
            ("pk" in data || "user" in data);
        //
        // Check if the data is undefined
        const is_undefined = data === undefined;
        //
        // Return true if the data is a user or undefined
        return is_user || is_undefined;
    }
    //
    // Administer the authoriser
    static async administer() {
        //
        // Open the authoriser in a fullscreen popup
        const result = await popup.administer_page(this.url, authoriser.validate_auth_message, {
            target: "_blank",
    ...
    get section() {
        ...
        const sections_map = this.sections;
        //
        ...
        const sections_iterator = sections_map.values();
        //
        //Convert the values (iterator) to an array
        const sections = Array.from(sections_iterator);
        //
        //Find the section that is current
        const section = sections.find((sect) => sect.is_current);
        //
        // If the section is not found, throw an error
        if (!section)
            throw new mutall_error("No current section not found");
        //
    ...
    get other_sections() {
        //
        //Find the map of sections of an authoriser
        const sections_map = this.sections;
        //
        //The map must be set by now.
        if (!sections_map)
        ...
        const other_sections = Array.from(this.sections.values()).filter((sect) => !sect.is_current);
...
export class section extends io_parent {
    fieldset;
    //
    ...
    async check_inputs() {
        ...
        for (const Io of this.ios.values()) {
            ...
            const caption = Io.search_option("caption");
            //
            ...
            //available
            const msg = `${caption ?? name} is required`;
            //
...
export class forgot extends section {
    ...
    async authenticate(curr_user) {
        ...
        const result = await this.email_password(password, curr_user);
        //
        //Kill teh system if the emial was nt sent
        if (result !== "ok")
            throw new mutall_error(result.message);
        //
    ...
    async generate_new_password(curr_user) {
        ...
        //
        //Save the new password to the db using the exec method
        const result = await this.exec_php("questionnaire", ["mutall_users"], "load_common", [layout]);
        //
    ...
    async email_password(password, curr_user) {
        ...
        const body = `Dear ${curr_user.name},  your new password is ${password}`;
        //
        ...
        "mutall_mailer", [], "send_email", [email, subject, body, "Mutall Data"]);
        //
