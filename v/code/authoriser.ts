...
//
// Require this to be implemented for authoriser to collect user data
export interface Iauthoriser {
  //
  // Administer the authoriser, i.e. collect user data
...
export class authoriser extends popup<
  { name: string; pk: number } | undefined
> {
  //
  ...
  private static readonly html_file: string = "authoriser.html";
  //
  ...
  static validate_auth_message(data: unknown): data is user | undefined {
    //
  ...
  static async administer(): Promise<user | undefined> {
    ...
    const result = await popup.administer_page<user | undefined>(
      this.url,
      authoriser.validate_auth_message,
      {
        target: "_blank",
  ...
  static get user_prop(): user | undefined {
    //
