...
export class fluctuate {
  ...
  //radio button.
  static onclick(ref: HTMLElement) {
    //
    ...
    if (!mother)
      throw new mutall_error(
        "No data-field closest to the current reference element found",
        ref
      );
    //
  ...
  /*
    ...
    */
  static execute(mother: HTMLElement) {
    //
    ...
    if (dfname === undefined)
      throw new mutall_error(
        'An undentified (mother) field (using data-field attribute) cannot be "collapsed"',
        mother
      );
    //
    ...
    } = fluctuate.separate_children(mother, dna);
    //
    //If the elder is valid expand her children
    if (elder) fluctuate.summarise_child(elder, family_dna, false);
    //
    //For each sibbling, hide her children
    siblings.forEach((sibling) =>
      fluctuate.summarise_child(sibling, family_dna, true)
    );
  }
  ...
  static summarise_child(
    ...
    const dna: { elder: string; siblings?: string } = { elder: family_dna };
    //
  ...
  static separate_children(
    mother: HTMLElement,
    dna: { elder: string; siblings?: string }
  ) {
    //Get all the children of the mother
    const children = <HTMLElement[]>Array.from(mother.children);
    //
  ...
  static get_elder(
    ...
    if (len > 1)
      throw new mutall_error(
        `This css '${css}' produces '${len}' elders`,
        organs
      );
    //
    ...
    if (elders.length == 0)
      throw new mutall_error(
        `Invalid form. No child qualifies to be an elder defined by css '${css}'`
      );
    //
    //It can only be one
    if (elders.length > 1)
      throw new mutall_error(
        `Invalid form. Found ${elders.length} elders for css '${css}'`,
        elders
      );
    //
  ...
  static get_siblings(
    ...
    if (!dna.elder && !dna.siblings)
      throw new mutall_error(
        `Invalid form. At least the elder or sibling (css) dna must be availale`
      );
    //
    ...
    const organs = <HTMLElement[]>Array.from(mother.querySelectorAll(css));
    //
    //A sibling is a child that contains any of these organs.
    const siblings = children.filter((child) =>
      fluctuate.contains(child, organs)
    );
    //
    //Return the siblings
    return siblings;
  ...
  static contains(child: HTMLElement, organs: HTMLElement[]) {
    //
    ...
    throw new mutall_error(
      `Invalid form. A child contains ${count} organs`,
      child
    );
  }
}
