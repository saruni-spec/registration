...
export class authoriser extends view {
  ...
  static async administer():Promise<user|undefined>{
    //
    //Create a new window
    const popup:Window|null = window.open(this.url,'_blank','fullscreen=yes');
    //
  ...
  get section(): section {
    ...
    if (!sections_map) throw new mutall_error('Authoriser sections are not set yet');
    //
    ...
    const section: section|undefined =sections.find((sect) => sect.is_current);
    //
