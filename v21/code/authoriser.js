...
export class authoriser extends view {
    ...
    set_sections() {
        ...
        radios.forEach((radio) => {
            radio.onclick = (evt) => this.clear_errors();
        });
    }
    //
    // Create the sections from the identified fieldsets in the html
    ...
    async authorise(event) {
        ...
        if (!opener) {
            ...
            throw new mutall_error("Opener not found. Check console.log for user_data");
        }
        //
        //Post the user data
    ...
    static async administer() {
        //
        //Create a new window
        const popup = window.open(this.url);
        //
        //Abort if the openning failed
        if (!popup)
            throw new mutall_error(`Window opening failed`);
        //
        ...
        const user = await new Promise((resolve) => {
            const messageHandler = (evt) => {
                ...
                if (typeof evt.data === "object" &&
                    ...
                    "name" in evt.data &&
                    ("pk" in evt.data ||
                        "user" in evt.data) /* depending on your user_data structure */) {
                    ...
                    // It's a message, but not the one we're waiting for (e.g., from dev tools)
