...
export interface Iauthoriser {
  administer(): Promise<user | undefined>;
}
...
export class authoriser extends view {
  ...
  //
  // Cached URL for authoriser.html
  private static _authoriserHtmlConcreteUrl: string | null = null;

  /**
   * Gets the absolute URL for authoriser.html, assuming it's in the same directory as authoriser.js.
   * Uses import.meta.url to resolve the path relative to the current module.
   */
  static getHtmlUrl(): string {
    if (this._authoriserHtmlConcreteUrl === null) {
      try {
        // import.meta.url is the full URL of the current JavaScript module (authoriser.js)
        // new URL('authoriser.html', import.meta.url) constructs an absolute URL
        // for 'authoriser.html' by resolving it relative to the location of authoriser.js.
        this._authoriserHtmlConcreteUrl = new URL(
          "authoriser.html",
          import.meta.url
        ).href;
      } catch (e) {
        console.error(
          "Authoriser: Error constructing URL for authoriser.html using import.meta.url:",
          e
        );
        // Fallback if import.meta.url is not available or fails.
        // This fallback will likely exhibit the original problem in consuming apps.
        this._authoriserHtmlConcreteUrl = "authoriser.html";
        // Consider throwing an error here or having a more robust fallback
        // if the component cannot function without its HTML template.
      }
    }
    return this._authoriserHtmlConcreteUrl;
  }

  // Static property 'url' to be used by window.open
  // This ensures backward compatibility if 'authoriser.url' was used directly before.
  static get url(): string {
    return this.getHtmlUrl();
  }
  //
  //Define  some local window
  ...
  async authorise(event: Event): Promise<void> {
    ...
    window.localStorage.setItem(authoriser.user_key, JSON.stringify(user_data));
    //
    //Post the user (data) to the window that opened this popup
    const opener: any = window.opener;
    //
    //Ensure the opener is available; if not console log the user and report error
    if (!opener) {
      console.log(user_data);
      throw new mutall_error(
        "Opener not found. Check console.log for user_data"
      );
    }
    //
    //Post the user data
    opener.postMessage(user_data);
  }
  //
  //Administer the authorisation process from a caller application
  static async administer(): Promise<user | undefined> {
    //
    //Create a new window
    const popup: Window | null = window.open(this.url);
    //
    //Abort if the openning failed
    if (!popup) throw new mutall_error(`Window opening failed`);
    //
    //Wait for the popup window to post the user (data)
    const user: user = await new Promise((resolve) => {
      //
      // Handle messages from the popup
      // Check if the message is coming from where you expect it to and if the structure
      // of the message is what you expect
      const messageHandler = (evt: MessageEvent) => {
        //
        // 1. Check origin (important for security if origins can differ)
        // Reject promises from unauthorised origins
        //
        // 2. Check if the data has an expected structure
        if (
          typeof evt.data === "object" &&
          evt.data !== null &&
          "name" in evt.data &&
          ("pk" in evt.data || "user" in evt.data)
        ) {
          console.log("Valid user data received from popup:", evt.data);
          //
          //Remove the event listener
          window.removeEventListener("message", messageHandler);
          //
          //
          resolve(evt.data as user);
        } else {
          // It's a message, but not the one we're waiting for
          console.log("Received non-user message, ignoring:", evt.data);
          //
          // Do nothing
        }
      };
      //
      // Add the message handler to the window
  ...
  //
  //Returns the current section by looking up the the one that marked as 'is_current'
  get section(): section {
    //
    //Get the map of sections of an authoriser
    const sections_map: mymap<section_id, section> | undefined = this.sections;
    //
    //The map must be set by now.
    if (!sections_map)
      throw new mutall_error("Authoriser sections are not set yet");
    //
    //Pick the section (values) from the map
    const sections_iterator: MapIterator<section> = sections_map.values();
    //
    //Convert the values (iterator) to an array
    const sections: Array<section> = Array.from(sections_iterator);
    //
    //Find the section that is current
    const section: section | undefined = sections.find(
      (sect) => sect.is_current
    );
    //
    // If the section is not found, throw an error
    if (!section) throw new mutall_error("No current section not found");
    //
    //Return teh found section
    return section;
  }
  //
...
export abstract class section extends io_parent {
  ...
  //for each section. If checked, then it is the current; otherwise it is not
  public get is_current(): boolean {
    ...
    //
    //The checked status of the element determines the result
  ...
  async check_inputs(): Promise<boolean> {
    ...
    for (const Io of this.ios.values()) {
      ...
      const caption: string | undefined = Io.search_option("caption");
      //
      ...
      //available
      const msg: string = `${caption ?? name} is required`;
      //
