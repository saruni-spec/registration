...
export class auth extends view {
        ...
        sections.forEach((section_name) => {
            ...
            if (section && section !== selected_fieldset) {
                ...
                label_sections.forEach((label_section) => {
                    ...
                    const errorSpan = label_section.querySelector(".error");
                    //
                    // Clear the error message in the error span
                    if (errorSpan) {
    ...
    #check_sign_up() {
        ...
        const css = 'label:has(span:contains("Password")) input[type="password"]';
        //
        //get the password field
        const password = document.querySelector(css);
        if (!password)
        ...
        //
        //css selector to get the password field
        const confirm_css = 'label:has(span:contains("Confirm Password")) input[type="password"]';
        //
        //get the password field
        const confirm_password = document.querySelector(confirm_css);
        if (!confirm_password)
    ...
    #check_change() {
        ...
        //
        //css selector to get the password field
        const css = 'label:has(span:contains("Old Password")) input[type="password"]';
        //
        //get the password field
        const old_password = document.querySelector(css);
        if (!old_password)
        ...
        //
        //css selector to get the password field
        const new_password_css = 'label:has(span:contains("New Password")) input[type="password"]';
        //
        //get the password field
        const new_password = document.querySelector(new_password_css);
        if (!new_password)
            throw new mutall_error("password field is missing");
        //
        //check if the password field has an input
    ...
    #get_change() {
        ...
        //
        //css selector to get the password field
        const password_css = 'label:has(span:contains("Password")) input[type="password"]';
        //
        //get the password field
        const password = document.querySelector(password_css);
        if (!password)
            throw new mutall_error("password field is missing");
        //
        //css selector to get the new password field
        const cp_css = 'label:has(span:contains("New Password")) input[type="password"]';
        //
        //get the password field
        const new_password = document.querySelector(cp_css);
        if (!new_password)
            throw new mutall_error("password field is missing");
        return {
            operation: "change",
    ...
    async #login(credentials) {
        ...
        if (!current_user) {
            ...
            const error_span = this.section.querySelector(`label:has(span:contains("User Name")) span.error`);
            if (!error_span)
            ...
            const password_error_span = this.section.querySelector(`label:has(span:contains("Password")) span.error`);
    ...
    async #forgot_password(credentials) {
        ...
        if (!current_user) {
            ...
            const email_error = this.section.querySelector(`label:has(span:contains("Email")) span.error`);
            if (!email_error)
    ...
    async #change_password(credentials) {
        ...
        if (!current_user) {
            ...
            //
            //get the error span for the password
            const password_error_span = this.section.querySelector(`label:has(span:contains("Old Password")) span.error`);
            if (!password_error_span)
