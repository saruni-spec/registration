...
export class auth extends view {
  ...
  clear_errors(): void {
    ...
    const form_error = document.getElementById("form_error") as HTMLSpanElement;
    //
  ...
  #check_inputs(): HTMLElement | undefined {
    //
  ...
  #check_login(section: HTMLElement): HTMLElement | undefined {
    //
    //check if all login fields have a value and report the error if an input
    // is missing
    if (!this.#validate_required_fields(section)) return;
    return section;
  }

  // Check sign up details with password validation
  #check_sign_up(section: HTMLElement): HTMLElement | undefined {
    //
    ...
    const passwords = section.querySelectorAll(
      "input[type=password]"
  ...
  #check_forgot(section: HTMLElement): HTMLElement | undefined {
    //
  ...
  #check_change(section: HTMLElement): HTMLElement | undefined {
    //
  ...
  #get_inputs(field_set: HTMLElement): credentials {
    //
    //get the current operation
    const operation = field_set.id as operation_id;
    //
    //Get the name of the user
    const name: HTMLInputElement | null =
      field_set.querySelector("input[type=text]");
    if (!name) {
      const message: string = "Name field is missing";
      alert(message);
  ...
  #get_credentials(): credentials | undefined {
    ...
    const inputs: HTMLElement | undefined = this.#check_inputs();
    //
    //return undefined if the chck fails
    if (!inputs) return undefined;
    //
    // Collect the credentials
    const credentials: credentials = this.#get_inputs(inputs);
    return credentials;
  }
  //
  ...
  #show_error(operation: operation_id, fieldName: string, error: string): void {
    //
    // Find the current input field by using its parent fieldset
    const current_input = document.querySelector(
      `#${operation} input[type="${fieldName}"]`
    ) as HTMLInputElement;
    if (!current_input) return;

    // Get the error span that directly follows this input
    const errorSpan = current_input.nextElementSibling as HTMLSpanElement;
    if (errorSpan) {
