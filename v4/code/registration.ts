...
export class auth extends view {
  ...
  #get_section(): HTMLElement {
    ...
    const choice: string = this.#get_operation_id(form);
    //
    ...
    switch (section.id) {
      ...
      default:
        throw new mutall_error(
          `This sction id:${section.id} is not an opration id`
        );
    }
    return result;
  }
  ...
  #check_login(section: HTMLElement): operation_id | undefined {
    ...
    const name_input: HTMLInputElement = <HTMLInputElement>(
      this.get_element("login_name")
    );
    //
    // report the error if the input is an empty string
    if (name_input.value.trim() === "") {
      const name_error: HTMLSpanElement = this.get_element("ln_error");
      name_error.textContent = "This field is required";
      return;
    }
    //
    //check if the password has a value
    const password_input: HTMLInputElement = <HTMLInputElement>(
      this.get_element("login_password")
    );
    //
    //report that the password is required if the input is empty
    if (password_input.value.trim() === "") {
      const password_error: HTMLSpanElement = this.get_element("ln_error");
  ...
  #check_sign_up(section: HTMLElement): operation_id | undefined {
    ...
    // Check username
    const name_input: HTMLInputElement = <HTMLInputElement>(
      this.get_element("sign_up_name")
    );
    if (name_input.value.trim() === "") {
      const name_error: HTMLSpanElement = this.get_element("sn_error");
      name_error.textContent = "This field is required";
      return;
    }
    //
    // Check email
    const email_input: HTMLInputElement = <HTMLInputElement>(
      this.get_element("sign_up_email")
    );
    if (email_input.value.trim() === "") {
      const email_error: HTMLSpanElement = this.get_element("se_error");
    ...
    if (password.value.trim() === "") {
      const password_error: HTMLSpanElement = this.get_element("sp_error");
      password_error.textContent = "This field is required";
      return;
    }
    //
    ...
    if (confirm_password.value.trim() === "") {
      const confirm_error: HTMLSpanElement = this.get_element("sc_error");
  ...
  #check_forgot(section: HTMLElement): operation_id | undefined {
    ...
    // Check username
    const name_input: HTMLInputElement = <HTMLInputElement>(
      this.get_element("forgot_name")
    );
    if (name_input.value.trim() === "") {
      const name_error: HTMLSpanElement = this.get_element("fn_error");
      name_error.textContent = "This field is required";
      return;
    }
    //
    // Check email
    const email_input: HTMLInputElement = <HTMLInputElement>(
      this.get_element("forgot_email")
    );
    if (email_input.value.trim() === "") {
      const email_error: HTMLSpanElement = this.get_element("fe_error");
      email_error.textContent = "This field is required";
      return;
    }
    //
  ...
  #check_change(section: HTMLElement): operation_id | undefined {
    ...
    // Check username
    const name_input: HTMLInputElement = <HTMLInputElement>(
      this.get_element("change_name")
    );
    if (name_input.value.trim() === "") {
      const name_error: HTMLSpanElement = this.get_element("cn_error");
      name_error.textContent = "This field is required";
      return;
    }
    //
    // Check old password
    const old_password: HTMLInputElement = <HTMLInputElement>(
      this.get_element("change_password")
    );
    if (old_password.value.trim() === "") {
      const old_password_error: HTMLSpanElement = this.get_element("cp_error");
    ...
    if (new_password.value.trim() === "") {
      const new_password_error: HTMLSpanElement = this.get_element("cnp_error");
    ...
    if (confirm_password.value.trim() === "") {
      const confirm_error: HTMLSpanElement = this.get_element("cc_error");
      confirm_error.textContent = "This field is required";
      return;
    }
    //
  ...
  #get_inputs(operation: operation_id): credentials {
    ...
    switch (operation) {
      ...
      case "sign_up": {
        ...
        const password = this.get_element(
          "sign_up_password"
        ) as HTMLInputElement;
        //
  ...
  async #login(credentials: credentials): Promise<user | undefined> {
    ...
    if (
      await this.exec_php("mutall", [], "password_verify", [
  ...
  async #reset_password(credentials: credentials): Promise<user | undefined> {
    //
